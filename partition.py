...
if __name__ == '__main__':
    ...
    fname_suffix = mbtiles_fname[:-len('.mbtiles')]

    to_partition_file = Path('partition_info.json')
    ...
    mosaic_data = create_pmtiles(partition_info, f'{fname_suffix}_', reader)
    pprint(mosaic_data)
    Path(f'{fname_suffix}.mosaic.json').write_text(json.dumps(mosaic_data))
